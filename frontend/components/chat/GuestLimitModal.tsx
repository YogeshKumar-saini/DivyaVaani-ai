    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input
          ...
          className="h-12 w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 text-[15px] text-white placeholder:text-white/20 focus:border-violet-400/50 focus:bg-white/[0.07] focus:outline-none focus:ring-1 focus:ring-violet-500/20 transition-all duration-200 pr-10"
        />
        {rightElement && (
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="absolute top-5 right-5 z-30 w-9 h-9 rounded-full flex items-center justify-center text-white/30 hover:text-white/80 transition-all duration-200 group"
                style={{
                    ...
                    {!isForgotPassword && (
                      ...
                      >
                        <motion.div
                          ...
                          style={{
                            width: "calc(50% - 6px)",
                            left: activeTab === "login" ? "calc(50% + 2px)" : "4px",
                          ...
                          <CustomGoogleButton
                            onSuccess={async (credential) => {
                              try {
                                ...
                                const userData = await authService.me(response.access_token);
                                await googleLogin(credential, { redirectTo: false });
                                onAuthSuccess?.(userData.id);
                                onClose();
                              } catch (e) {
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <input
          ...
          minLength={minLength}
          placeholder={placeholder}
          className="h-12 w-full rounded-xl border border-white/10 bg-white/[0.04] px-4 pr-10 text-[15px] text-white placeholder:text-white/20 focus:border-violet-400/50 focus:bg-white/[0.07] focus:outline-none focus:ring-1 focus:ring-violet-500/20 transition-all duration-200"
        />
        <button
          type="button"
          onClick={onToggle}
...
function CustomGoogleButton({
  ...
  return (
    <div className="relative group w-full">
      {/* Premium glow effect */}
      <div className="absolute -inset-0.5 bg-gradient-to-r from-violet-500/20 to-indigo-500/20 rounded-2xl blur opacity-0 group-hover:opacity-100 transition duration-500"></div>

      {/* Visible custom button content */}
      <button
        type="button"
        onClick={() => login()}
        className="relative flex items-center justify-center w-full h-[50px] rounded-xl bg-white/[0.04] border border-white/10 group-hover:border-white/20 group-hover:bg-white/[0.06] transition-all duration-300 cursor-pointer"
      >
